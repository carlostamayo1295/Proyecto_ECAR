...
public static class DataSeeder
{
    ...
    private static async Task SeedEquipmentCatalogAsync(ECARDbContext context)
    {
        ...
        var categoryNames = await context.CategoriasEquipo.Select(c => c.Nombre).ToListAsync();
        var missingEquipmentCategories = equipmentCategories
            .Where(required => !categoryNames.Contains(required.Nombre, StringComparer.OrdinalIgnoreCase))
            .ToList();
        if (missingEquipmentCategories.Count > 0)
        {
            await context.CategoriasEquipo.AddRangeAsync(missingEquipmentCategories);
            await context.SaveChangesAsync();
        }
