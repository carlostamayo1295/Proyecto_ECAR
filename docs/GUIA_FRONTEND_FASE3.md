...
- **`IniciarInspeccionAsync` puede responder 409 y aun así traer datos válidos.** Si el técnico
  ya tiene una inspección en curso para ese equipo, el API responde **HTTP 409 con la
  inspección existente en `Data` y `Success = true`** (el mensaje lo explica). FE-3 debe
  continuarla navegando a `Data.IdInspeccion`, no mostrar un error. En la práctica basta con
  comprobar `result?.Data != null`, que cubre por igual el alta nueva y la continuación.
